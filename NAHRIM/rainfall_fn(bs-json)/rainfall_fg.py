...
MAX_RETRIES = 3  # retries per state
...
def parse_state_page(html_text, state_code, state_name, logger):
    ...
    if len(rows) < 3:
        logger.warning(
            f"[RAIN] {state_name}: expected at least 3 rows (2 header + data) "
            f"in table, found {len(rows)}"
        )
        return []
...
def items_to_csv(all_items):
    ...
    Field order: state_code, state_name, then all other keys in discovery order.
    """
    if not all_items:
        return ""

    # Build fieldnames in a stable order
    fieldnames = []
    ...
    # Move state_code and state_name to the front if present
