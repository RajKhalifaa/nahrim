...
def upload_to_obs(content_str, logger):
    """
    Upload the CSV to OBS in two ways:
      1) Timestamped file  (for archive)
      2) Fixed 'latest' file (for CDM job)
    Returns both keys.
    """
    ...
    ts_key = f"{OBS_OBJECT_PREFIX}rainfall_trend_{timestamp}.csv"
    logger.info(f"[OBS] Uploading archive CSV to bucket={OBS_BUCKET}, key={ts_key}")
    resp1 = obs_client.putContent(OBS_BUCKET, ts_key, content_str)
    if resp1.status >= 300:
        raise RuntimeError(
            f"OBS upload (archive) failed: status={resp1.status}, "
            f"errorCode={getattr(resp1, 'errorCode', '')}, "
            f"errorMessage={getattr(resp1, 'errorMessage', '')}"
        )

    # 2) fixed name – always overwritten
    latest_key = f"{OBS_OBJECT_PREFIX}rainfall_trend_latest.csv"
    logger.info(f"[OBS] Uploading latest CSV to bucket={OBS_BUCKET}, key={latest_key}")
    resp2 = obs_client.putContent(OBS_BUCKET, latest_key, content_str)
    if resp2.status >= 300:
        raise RuntimeError(
            f"OBS upload (latest) failed: status={resp2.status}, "
            f"errorCode={getattr(resp2, 'errorCode', '')}, "
            f"errorMessage={getattr(resp2, 'errorMessage', '')}"
        )

    logger.info("[OBS] Uploads successful")
    return ts_key, latest_key
...
def handler(event, context):
    ...
    # Upload to OBS
    ts_key, latest_key = upload_to_obs(csv_body, logger)

    # Start CDM job (rainfall_test_API) after upload
    job_name = os.getenv("CDM_JOB_NAME", "rainfall_test_API")
    ...
    result = {
        ...
        "bucket": OBS_BUCKET,
        "object_key_archive": ts_key,
        "object_key_latest": latest_key,
        "cdm_job": cdm_result,
    }
